# -*- coding: utf-8 -*-
"""
Classify All Database Stocks (classify_all_database.py)
Loads all 1980+ stocks from stocks_list.txt, fetches their industry info in parallel,
caches them, and generates a master industry map.
"""

import os
import json
import sys
import io
import re
from datetime import datetime
from concurrent.futures import ThreadPoolExecutor, as_completed
import yfinance as yf

# Standardize output encoding
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')
sys.stderr = io.TextIOWrapper(sys.stderr.buffer, encoding='utf-8')

# Constants
STOCK_LIST_FILE = "stocks_list.txt"
CACHE_FILE = "industry_cache.json"
REPORT_MD = "all_stocks_industry_map.md"
REPORT_HTML = "all_stocks_industry_map.html"

# Expert Categorization Database (reused from analyze_trends.py)
STOCK_SUBCLASS = {
    "AES-KY": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "AMAX-KY": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "GOGOLOOK": ('軟體與資訊服務', '數位信任科技 (Whoscall)'),
    "IKKA-KY": ('半導體與 PCB 設備/材料', '精密衝壓與精密金屬組件'),
    "M31": ('半導體產業', 'IC設計 - 矽智財與 ASIC (IP/ASIC)'),
    "TPK-KY": ('光電與顯示面板', '光電與顯示零組件與材料'),
    "三一東林": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "三星": ('車用與傳統工業', '工具與金屬配件 - 螺絲螺帽與緊固件'),
    "三洋實業": ('建材營造與房地產', '營建與不動產開發'),
    "三聯": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "三芳": ('綠能、環保與化學工業', '石化原料與特用化學品'),
    "上詮": ('通訊、線材與連接器', '光通訊與光模組'),
    "世坤": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "世德": ('車用與傳統工業', '工具與金屬配件 - 螺絲螺帽與緊固件'),
    "世紀*": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "世紀鋼": ('建材營造與房地產', '鋼構工程與離岸風電'),
    "世紀風電": ('綠能、環保與化學工業', '太陽能綠能'),
    "世芯-KY": ('半導體產業', 'IC設計 - 矽智財與 ASIC (IP/ASIC)'),
    "世豐": ('車用與傳統工業', '工具與金屬配件 - 螺絲螺帽與緊固件'),
    "世鎧": ('車用與傳統工業', '工具與金屬配件 - 螺絲螺帽與緊固件'),
    "中工": ('建材營造與房地產', '營建與不動產開發'),
    "中環": ('工業電腦與電腦週邊', '電腦周邊 - 光碟與光儲存媒體'),
    "中砂": ('半導體與 PCB 設備/材料', '半導體鑽石碟與再生晶圓'),
    "中磊": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "中纖": ('綠能、環保與化學工業', '人造纖維與石化原料 (EG/EO)'),
    "中美晶": ('半導體產業', '矽晶圓/太陽能'),
    "中興電": ('電子零組件與其它', '馬達、變壓器與重電設備'),
    "中華化": ('綠能、環保與化學工業', '基礎及特用化學品 (酸鹼化學)'),
    "中釉": ('半導體與 PCB 設備/材料', '陶瓷釉料與半導體材料'),
    "中鋼": ('鋼鐵與金屬', '綜合鋼鐵廠'),
    "中鋼構": ('建材營造與房地產', '鋼構工程與離岸風電'),
    "久威": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "久裕": ('生技醫療', '醫藥通路與連鎖藥局'),
    "久陽精密": ('車用與傳統工業', '螺絲螺帽與環保資源回收'),
    "九暘": ('半導體產業', 'IC設計 - 聯網與射頻晶片'),
    "九豪": ('被動元件與石英元件', '被動元件上游材料與設備'),
    "九豪精密": ('被動元件與石英元件', '電阻陶瓷基板 (被動上游)'),
    "九齊": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "事欣科": ('軍工與航太', '軍工與博弈工業電腦/系統整合'),
    "亞信": ('半導體產業', 'IC設計 - 聯網與射頻晶片'),
    "亞元": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "亞力電機": ('電子零組件與其它', '馬達、變壓器與重電設備'),
    "亞崴": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "亞德客-KY": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "亞泰": ('光電與顯示面板', '光電與光學鏡頭'),
    "亞泰金屬": ('半導體與 PCB 設備/材料', 'PCB/CCL塗佈機與設備'),
    "亞矽": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "亞翔": ('半導體與 PCB 設備/材料', '半導體廠務與設備工程'),
    "亞銳士": ('通訊、線材與連接器', '光通訊與光模組'),
    "亞電": ('PCB 與銅箔基板', '軟性銅箔基板 (FCCL) 與覆蓋膜'),
    "仁寶": ('伺服器與資訊週邊', '伺服器與筆電代工'),
    "今國光": ('光電與顯示面板', '玻璃/塑膠光學鏡片'),
    "今展科": ('被動元件與石英元件', '電感器與磁性元件'),
    "今皓": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "仲琦": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "伍豐": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "伯特光": ('車用與傳統工業', '精密機械與金屬加工 - 零組件加工'),
    "伯鑫": ('車用與傳統工業', '工具與金屬配件 - 手工具與氣動工具'),
    "佑華": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "佳必琪": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "佳穎": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "來頡": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "保綠-KY": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "保銳": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "信昌電陶": ('被動元件與石英元件', '陶瓷電容 (MLCC) 與介電粉末'),
    "信紘科": ('半導體與 PCB 設備/材料', '半導體廠務與設備工程'),
    "信邦": ('通訊、線材與連接器', '連接器 - 車用與工業連接器'),
    "信音": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "信驊": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "倉佑": ('車用與傳統工業', '汽車零組件 - 傳動與引擎動力零件'),
    "倉和": ('半導體與 PCB 設備/材料', '太陽能電池網版印刷網版'),
    "倍利科": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "倍微科技": ('軟體與資訊服務', '半導體與通訊軟硬體通路商'),
    "倚天酷碁-創": ('工業電腦與電腦週邊', '電腦周邊 - 掃描器與影像擷取'),
    "倚強科": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "倫飛": ('工業電腦與電腦週邊', '品牌電腦與主機板/顯示卡'),
    "偉聯": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "偉訓": ('工業電腦與電腦週邊', '電腦機殼與電源供應器'),
    "偉詮電": ('半導體產業', 'IC設計 - 微控制器 (MCU)'),
    "健信": ('車用與傳統工業', '精密機械與金屬加工 - 零組件加工'),
    "健和興": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "健椿": ('車用與傳統工業', '精密機械與金屬加工 - 零組件加工'),
    "健鼎": ('PCB 與銅箔基板', '多層印刷電路板 (PCB)'),
    "傑霖科技": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "億而得": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "儒鴻": ('紡織與成衣', '機能性布料與成衣 OEM'),
    "優群": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "元創精密": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "元大金": ('金融服務', '金融控股公司 (證券/銀行/期貨)'),
    "兆勁": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "兆赫": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "先益": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "先進光": ('光電與顯示面板', '車用與筆電光學鏡頭'),
    "光寶科": ('通訊、線材與連接器', '光通訊與光模組'),
    "光環": ('通訊、線材與連接器', '光通訊與光模組'),
    "光聖": ('通訊、線材與連接器', '光通訊與光模組'),
    "光聯": ('通訊、線材與連接器', '光通訊與光模組'),
    "光菱": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "光譜": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "光鋐": ('通訊、線材與連接器', '光通訊與光模組'),
    "光錐": ('通訊、線材與連接器', '光通訊與光模組'),
    "光隆精密-KY": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "光頡": ('被動元件與石英元件', '電阻/電容/MLCC'),
    "光頡科技": ('被動元件與石英元件', '精密與薄膜電阻'),
    "光鼎": ('光電與顯示面板', 'LED封裝與照明元件'),
    "全友": ('工業電腦與電腦週邊', '影像掃描器與週邊'),
    "全台": ('被動元件與石英元件', '石英元件與頻率控制元件'),
    "全宇昕": ('半導體產業', '功率半導體封裝與設計'),
    "全新": ('通訊、線材與連接器', '光通訊與光模組'),
    "全球傳動": ('車用與傳統工業', '工具與金屬配件 - 傳動元件'),
    "全科": ('通訊、線材與連接器', '光通訊與光模組'),
    "全訊": ('通訊、線材與連接器', '光通訊與光模組'),
    "全譜": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "公準": ('車用與傳統工業', '工具與金屬配件 - 傳動元件'),
    "六方科-KY": ('車用與傳統工業', '精密機械與金屬加工 - 零組件加工'),
    "六暉-KY": ('車用與傳統工業', '汽車零組件 - 輪胎與橡膠'),
    "其陽": ('工業電腦與電腦週邊', '工業電腦 (IPC)'),
    "冠德": ('建材營造與房地產', '營建與不動產開發'),
    "冠軍": ('建材營造與房地產', '建材與裝修'),
    "凌華": ('工業電腦與電腦週邊', '工業電腦 (IPC) 與邊緣運算'),
    "凌通": ('半導體產業', 'IC設計 - 微控制器 (MCU)'),
    "凌陽": ('半導體產業', 'IC設計 - 多媒體與影音 SoC'),
    "凌陽創新": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "凡甲": ('通訊、線材與連接器', '連接器 - 高頻與光學連接器'),
    "凱基金": ('金融服務', '金融控股公司 (壽險/銀行/證券)'),
    "凱崴": ('車用與傳統工業', '工具與金屬配件 - 螺絲螺帽與緊固件'),
    "凱碩": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "凱美": ('被動元件與石英元件', '電解電容與薄膜電容'),
    "凱鈺": ('半導體產業', 'IC設計 - 聯網與射頻晶片'),
    "凱鈺科技": ('半導體產業', 'IC設計 - 類比與網通 IC'),
    "利勤": ('建材營造與房地產', '營建與不動產開發'),
    "利奇": ('車用與傳統工業', '精密機械與金屬加工 - 零組件加工'),
    "前鼎": ('通訊、線材與連接器', '光通訊與光模組'),
    "創威": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "創惟": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "創意": ('半導體產業', 'IC設計 - 矽智財與 ASIC (IP/ASIC)'),
    "創控": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "創新服務": ('軟體與資訊服務', '軟體外包與系統整合'),
    "創見": ('工業電腦與電腦週邊', '電腦周邊 - 記憶體與儲存模組'),
    "劍麟": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "力士": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "力成": ('半導體產業', '半導體封測 - 記憶體及系統級封裝'),
    "力旺": ('半導體產業', 'IC設計 - 矽智財與 ASIC (IP/ASIC)'),
    "力智": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "力特": ('被動元件與石英元件', '石英元件與頻率控制元件'),
    "力積電": ('半導體產業', '晶圓代工'),
    "力肯": ('車用與傳統工業', '工具與金屬配件 - 手工具與氣動工具'),
    "力致": ('工業電腦與電腦週邊', '散熱模組與元件'),
    "力達-KY": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "力銘": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "力領科技": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "力鵬": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "力麗": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "加百裕": ('電子零組件與其它', '電池模組與儲能系統'),
    "加高": ('被動元件與石英元件', '石英元件與頻率控制元件'),
    "加高電子": ('被動元件與石英元件', '石英元件 (晶體/振盪器)'),
    "勁豐": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "動力-KY": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "勤崴國際": ('軟體與資訊服務', '圖資與導航軟體'),
    "勤美": ('車用與傳統工業', '精密機械與金屬加工 - 零組件加工'),
    "勤誠": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "匯僑設計": ('建材營造與房地產', '空間設計與顧問服務'),
    "十銓": ('工業電腦與電腦週邊', '電腦周邊 - 記憶體與儲存模組'),
    "千如": ('被動元件與石英元件', '電感器與磁性元件'),
    "千如電機": ('被動元件與石英元件', '電感器'),
    "千附": ('建材營造與房地產', '環保工程與廠務水資源'),
    "千附精密": ('軍工與航太', '光電與航太精密機械加工'),
    "協易機": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "協益": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "南亞": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "南亞科": ('半導體產業', '記憶體製造 (DRAM)'),
    "南俊國際": ('伺服器與資訊週邊', '伺服器導軌與滑軌'),
    "南港": ('車用與傳統工業', '汽車零組件 - 輪胎與橡膠'),
    "南紡": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "南茂": ('半導體產業', '半導體封測 - 驅動 IC 與記憶體'),
    "南電": ('PCB 與銅箔基板', 'IC 載板 (ABF/BT)'),
    "博大": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "原相": ('半導體產業', 'IC設計 - 影像感測與光學 IC'),
    "友勁": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "友訊": ('通訊、線材與連接器', '網通 - 企業交換器與企業網路'),
    "友達": ('光電與顯示面板', '顯示面板與液晶螢幕'),
    "友鋮": ('車用與傳統工業', '工具與金屬配件 - 其它五金製品'),
    "台光電": ('PCB 與銅箔基板', '銅箔基板 (CCL)'),
    "台勝科": ('半導體產業', '矽晶圓材料'),
    "台化": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "台半": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "台南": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "台南企業": ('紡織與成衣', '成衣製造與品牌'),
    "台喬": ('綠能、環保與化學工業', '石化原料與特用化學品'),
    "台嘉碩": ('被動元件與石英元件', '石英元件與頻率控制元件'),
    "台微體": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "台揚": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "台新新光金": ('金融服務', '金融控股公司 (台新/新光)'),
    "台林": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "台汽電": ('綠能、環保與化學工業', '汽電共生與綠能開發'),
    "台灣半導體": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "台灣嘉碩": ('被動元件與石英元件', '石英元件與聲表面波濾波器 (SAW)'),
    "台灣大": ('通訊、線材與連接器', '電信與通訊服務'),
    "台灣精材": ('半導體與 PCB 設備/材料', '半導體設備與材料'),
    "台灣精銳": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "台燿科技": ('PCB 與銅箔基板', '銅箔基板 (CCL)'),
    "台端": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "台精瓷": ('光電與顯示面板', '光電與顯示零組件與材料'),
    "台翰": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "台聯電": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "台苯": ('綠能、環保與化學工業', '石化原料與特用化學品'),
    "台虹": ('PCB 與銅箔基板', '軟性銅箔基板 (FCCL) 與太陽能背板'),
    "台通": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "台達電": ('電子零組件與其它', '電源供應器與適配器'),
    "台鋼建設": ('建材營造與房地產', '營建與不動產開發'),
    "台麗": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "合勤控": ('通訊、線材與連接器', '網通 - 企業交換器與企業網路'),
    "合晶科技": ('半導體產業', '矽晶圓材料'),
    "合機": ('綠能、環保與化學工業', '重電與電線電纜'),
    "合邦": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "吉祥全": ('半導體產業', '發光二極體與記憶體代理'),
    "吉茂": ('車用與傳統工業', '汽車零組件 - 車體與外觀件'),
    "同協": ('車用與傳統工業', '精密機械與金屬加工 - 鑄造鍛造與合金外殼'),
    "同欣電": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "同致": ('車用與傳統工業', '汽車零組件 - 車用電子與感測器'),
    "君帆": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "君曜": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "和勤": ('車用與傳統工業', '精密機械與金屬加工 - 鑄造鍛造與合金外殼'),
    "和大": ('車用與傳統工業', '汽車零組件 - 傳動與引擎動力零件'),
    "和桐": ('綠能、環保與化學工業', '界面活性劑原料 (LAS/烷基苯)'),
    "和椿": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "和碩": ('工業電腦與電腦週邊', '筆電與消費電子代工'),
    "品安": ('工業電腦與電腦週邊', '電腦周邊 - 記憶體與儲存模組'),
    "哲固": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "啟發電": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "啟碁": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "喬福": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "喬鼎": ('工業電腦與電腦週邊', '電腦主機與伺服器系統'),
    "嘉基": ('通訊、線材與連接器', '光通訊與光模組'),
    "嘉晶": ('半導體與 PCB 設備/材料', '磊晶片材料'),
    "嘉澤": ('通訊、線材與連接器', '連接器 - 高頻與光學連接器'),
    "嘉裕": ('紡織與成衣', '成衣製造與品牌'),
    "嘉鋼": ('車用與傳統工業', '精密機械與金屬加工 - 鑄造鍛造與合金外殼'),
    "嘉雨思-創": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "固緯": ('工業電腦與電腦週邊', '電子測試與量測儀器'),
    "國巨": ('被動元件與石英元件', '電阻/電容/電感多合一龍頭'),
    "國巨*": ('被動元件與石英元件', '電阻/電容/MLCC'),
    "國產": ('建材營造與房地產', '營建與不動產開發'),
    "國眾": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "國碩": ('光電與顯示面板', '太陽能材料與導電漿'),
    "國精化": ('綠能、環保與化學工業', '光固化樹脂材料'),
    "國統": ('綠能、環保與化學工業', '環保綠能與水資源'),
    "圓剛": ('工業電腦與電腦週邊', '電腦周邊 - 掃描器與影像擷取'),
    "圓裕": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "地心引力": ('軟體與資訊服務', '數位廣告與行銷科技'),
    "堡達": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "堤維西": ('車用與傳統工業', '汽車零組件 - 車體與外觀件'),
    "士電": ('電子零組件與其它', '馬達、變壓器與重電設備'),
    "大中": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "大井泵浦": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "大億": ('車用與傳統工業', '汽車零組件 - 車體與外觀件'),
    "大億金茂": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "大同": ('電子零組件與其它', '馬達、變壓器與重電設備'),
    "大器": ('紡織與成衣', '紡織、紗線與布料'),
    "大國鋼": ('鋼鐵與金屬', '鋼鐵與金屬通路'),
    "大拓-KY": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "大東電": ('通訊、線材與連接器', '電線電纜與繞線材'),
    "大樹": ('生技醫療', '醫藥通路與連鎖藥局'),
    "大眾控": ('工業電腦與電腦週邊', '電腦主機與伺服器系統'),
    "大立光": ('光電與顯示面板', '高階光學鏡頭'),
    "大詠城": ('車用與傳統工業', '精密機械與金屬加工 - 零組件加工'),
    "大量": ('通訊、線材與連接器', '光通訊與光模組'),
    "大銀微系統": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "大魯閣": ('觀光餐旅與休閒娛樂', '運動與休閒娛樂'),
    "天二科技": ('被動元件與石英元件', '電阻/電容/MLCC'),
    "天剛": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "天品": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "天宇": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "天擎": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "天方能源": ('半導體產業', 'IC設計 - AI 加速器與邊緣運算晶片'),
    "天正國際": ('被動元件與石英元件', '被動元件上游材料與設備'),
    "天良": ('生技醫療', '西藥銷售與保健食品'),
    "天鈺": ('半導體產業', 'IC設計 - 顯示驅動與觸控 IC'),
    "天鉞電": ('電子零組件與其它', '電子零組件 - 安防與控制系統'),
    "太子": ('建材營造與房地產', '營建與不動產開發'),
    "太普高": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "太欣": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "太空梭": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "太設": ('建材營造與房地產', '營建與不動產開發'),
    "太赫": ('通訊、線材與連接器', '光通訊與光模組'),
    "奕力": ('半導體產業', 'IC設計 - 顯示驅動與觸控 IC'),
    "奕力-KY": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "奕圓": ('半導體產業', 'IC設計 - 顯示驅動與觸控 IC'),
    "威力暘-創": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "威潤": ('軟體與資訊服務', '車載衛星定位監控 (Telematics)'),
    "威盛": ('半導體產業', 'IC設計 - AI 加速器與邊緣運算晶片'),
    "威鋒電子": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "宇智": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "宇隆": ('半導體與 PCB 設備/材料', '精密衝壓與精密金屬組件'),
    "安力-KY": ('車用與傳統工業', '精密機械與金屬加工 - 鑄造鍛造與合金外殼'),
    "安勤": ('工業電腦與電腦週邊', '工業電腦 (IPC)'),
    "安可": ('光電與顯示面板', 'ITO 導電玻璃'),
    "安可光電": ('光電與顯示面板', 'ITO 導電玻璃'),
    "安可科技": ('半導體產業', 'IC設計 - 多媒體與影音 SoC'),
    "安國": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "安格": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "安瑞-KY": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "安碁": ('被動元件與石英元件', '石英元件與頻率控制元件'),
    "安碁科技": ('被動元件與石英元件', '石英元件 (晶體/振盪器)'),
    "安葆": ('綠能、環保與化學工業', '能源工程與電力系統'),
    "宏捷科": ('通訊、線材與連接器', '光通訊與光模組'),
    "宏旭-KY": ('車用與傳統工業', '汽車鈑金模具'),
    "宏易": ('車用與傳統工業', '精密機械與金屬加工 - 零組件加工'),
    "宏正": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "宏碩系統": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "宏致": ('通訊、線材與連接器', '連接器 - 車用與工業連接器'),
    "宏觀": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "宏遠": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "宏齊": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "宜鼎": ('工業電腦與電腦週邊', '電腦周邊 - 記憶體與儲存模組'),
    "宣德": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "宸曜": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "富世達": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "富喬": ('半導體與 PCB 設備/材料', 'PCB 玻纖紗與玻纖布'),
    "富強鑫": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "富榮綱": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "富爾特": ('軟體與資訊服務', '數位廣告與行銷科技'),
    "富田-創": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "富致": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "富驊": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "富鼎": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "寬魚國際": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "寶利徠": ('半導體產業', 'IC設計 - AI 加速器與邊緣運算晶片'),
    "寶德": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "尖點": ('半導體與 PCB 設備/材料', 'PCB 鑽針與鑽孔加工服務'),
    "尼克森": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "居易": ('通訊、線材與連接器', '網通 - 企業交換器與企業網路'),
    "展達": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "岱稜": ('車用與傳統工業', '精密機械與金屬加工 - 零組件加工'),
    "岳豐": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "崇友": ('半導體與 PCB 設備/材料', '特殊工業設備與公用電梯'),
    "崇越": ('半導體與 PCB 設備/材料', '半導體設備與材料'),
    "崧騰": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "崴寶": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "川湖": ('伺服器與資訊週邊', '伺服器導軌與滑軌'),
    "工信": ('建材營造與房地產', '營建與不動產開發'),
    "巧新": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "巨庭": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "巨有科技": ('半導體產業', 'IC設計 - 矽智財與 ASIC (IP/ASIC)'),
    "巨漢": ('建材營造與房地產', '營建與不動產開發'),
    "巨虹": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "巨鎧精密-創": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "巨騰-DR": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "帆宣": ('半導體與 PCB 設備/材料', '半導體廠務與設備工程'),
    "希華": ('被動元件與石英元件', '石英元件與頻率控制元件'),
    "帝寶": ('車用與傳統工業', '汽車零組件 - 車體與外觀件'),
    "常珵": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "幃翔": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "年興": ('紡織與成衣', '成衣製造與品牌'),
    "幸康": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "康全電訊": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "康控-KY": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "康聯訊": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "康舒": ('電子零組件與其它', '電源供應器與適配器'),
    "廣宇": ('通訊、線材與連接器', '高頻連接器與傳輸線材'),
    "廣明": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "廣積": ('工業電腦與電腦週邊', '工業電腦 (IPC)'),
    "廣穎": ('工業電腦與電腦週邊', '記憶體模組與快閃碟'),
    "廣華-KY": ('車用與傳統工業', '汽車零組件 - 車體與外觀件'),
    "廣豐": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "廣達": ('伺服器與資訊週邊', '伺服器與筆電代工'),
    "廣錠": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "廣閎科": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "廣隆": ('電子零組件與其它', '電池模組與儲能系統'),
    "建國": ('建材營造與房地產', '營建與不動產開發'),
    "建大": ('車用與傳統工業', '汽車零組件 - 輪胎與橡膠'),
    "建德工業": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "建暐": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "建漢": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "建碁": ('工業電腦與電腦週邊', '電腦主機與伺服器系統'),
    "建舜電": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "建通": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "弘塑": ('半導體與 PCB 設備/材料', '半導體設備與材料'),
    "強信-KY": ('車用與傳統工業', '工具與金屬配件 - 其它五金製品'),
    "強新": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "強茂": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "彩晶": ('光電與顯示面板', '中小尺寸顯示面板'),
    "彬台": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "微矽電子-創": ('半導體產業', 'IC測試及封裝 - 功率元件測試'),
    "德勝": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "德律": ('半導體與 PCB 設備/材料', '量測儀器與精密檢測'),
    "德微": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "德昌": ('建材營造與房地產', '營建與不動產開發'),
    "德晉": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "德麥": ('傳統工業與傳統材料', '食品與麵包烘焙原料'),
    "志豐": ('電子零組件與其它', '電子零組件 - 聲學與感測元件'),
    "怡利電": ('車用與傳統工業', '汽車零組件 - 車用電子與感測器'),
    "恒耀": ('車用與傳統工業', '工具與金屬配件 - 螺絲螺帽與緊固件'),
    "愛山林": ('建材營造與房地產', '營建與不動產開發'),
    "愛普*": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "慶騰精密": ('車用與傳統工業', '粉末冶金與車用齒輪零件'),
    "應廣": ('半導體產業', 'IC設計 - 微控制器 (MCU)'),
    "成霖": ('傳統工業與其它', '廚衛五金'),
    "所羅門": ('半導體產業', 'IC設計 - AI 加速器與邊緣運算晶片'),
    "承啟": ('工業電腦與電腦週邊', '品牌電腦與主機板/顯示卡'),
    "技嘉": ('工業電腦與電腦週邊', '品牌電腦與主機板/顯示卡'),
    "振曜": ('通訊、線材與連接器', '網通 - 電子書與智慧電子裝置'),
    "振樺電": ('工業電腦與電腦週邊', 'POS 與自助服務終端'),
    "振發": ('車用與傳統工業', '精密機械與金屬加工 - 零組件加工'),
    "捷敏-KY": ('半導體產業', '半導體封測 - 功率半導體'),
    "捷波": ('工業電腦與電腦週邊', '品牌電腦與主機板/顯示卡'),
    "捷流閥業": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "揚博": ('半導體與 PCB 設備/材料', 'PCB與半導體濕製程設備/代理'),
    "揚智": ('半導體產業', 'IC設計 - 多媒體與影音 SoC'),
    "撼訊": ('工業電腦與電腦週邊', '顯示卡與電腦週邊'),
    "擎亞": ('半導體產業', '半導體代理通路'),
    "攸泰科技": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "敦泰": ('半導體產業', 'IC設計 - 顯示驅動與觸控 IC'),
    "敬鵬": ('PCB 與銅箔基板', '車用印刷電路板 (PCB)'),
    "數泓科": ('車用與傳統工業', '工具與金屬配件 - 手工具與氣動工具'),
    "斐成": ('建材營造與房地產', '營建與不動產開發'),
    "新代": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "新唐": ('半導體產業', 'IC設計 - 微控制器 (MCU)'),
    "新巨": ('電子零組件與其它', '電源供應器'),
    "新建": ('建材營造與房地產', '營建與不動產開發'),
    "新日興": ('車用與傳統工業', '精密機械與金屬加工 - 轉軸與機構件'),
    "新普": ('電子零組件與其它', '電池模組與儲能系統'),
    "新漢": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "新盛力": ('車用與傳統工業', '鋰電池模組 (LEV與儲能電池模組)'),
    "新纖": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "新至陞": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "新門": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "新麥": ('半導體與 PCB 設備/材料', '特殊工業設備與公用電梯'),
    "方土昶": ('半導體產業', '電子零組件代理商'),
    "方土霖": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "日勝生": ('建材營造與房地產', '營建與不動產開發'),
    "日成-KY": ('觀光餐旅與休閒娛樂', '娛樂與休閒'),
    "日月光投控": ('半導體產業', '半導體封測 - 晶圓級/全方位封測龍頭'),
    "日電貿": ('被動元件與石英元件', '被動元件通路商'),
    "旭源": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "旭然": ('綠能、環保與化學工業', '水處理過濾設備與濾心'),
    "旭隼": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "旺玖": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "旺矽科技": ('半導體與 PCB 設備/材料', '半導體探針卡與檢測設備'),
    "昆盈": ('工業電腦與電腦週邊', '電腦周邊 - 鍵盤與輸入裝置'),
    "昇佳電子": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "昇達科": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "昇陽半導體": ('半導體產業', '再生晶圓與薄化'),
    "明泰": ('通訊、線材與連接器', '網通 - 企業交換器與企業網路'),
    "明遠精密": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "星亞": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "星通": ('通訊、線材與連接器', '光通訊與光模組'),
    "星雲": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "映泰": ('工業電腦與電腦週邊', '主機板、顯示卡'),
    "映興": ('通訊、線材與連接器', '連接器 - 車用與工業連接器'),
    "春源": ('鋼鐵與金屬', '特殊鋼與鋼材加工'),
    "昭輝": ('車用與傳統工業', '汽車零組件 - 車體與外觀件'),
    "昶昕": ('綠能、環保與化學工業', 'PCB特用化學品與銅回收'),
    "昶瑞機電": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "時碩工業": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "晟楠": ('車用與傳統工業', '精密機械與金屬加工 - 零組件加工'),
    "普安": ('工業電腦與電腦週邊', '企業級磁碟存儲系統'),
    "普格": ('半導體產業', 'IC設計 - 記憶體控制 IC'),
    "普萊德": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "普誠": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "景碩": ('PCB 與銅箔基板', 'IC 載板 (BT/ABF)'),
    "晶宏": ('半導體產業', 'IC設計 - 顯示驅動與觸控 IC'),
    "晶心科": ('半導體產業', 'IC設計 - 矽智財與 ASIC (IP/ASIC)'),
    "晶技": ('被動元件與石英元件', '石英元件與頻率控制元件'),
    "晶焱": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "晶相光": ('半導體產業', 'IC設計 - 矽智財與 ASIC (IP/ASIC)'),
    "晶豪科": ('半導體產業', 'IC設計 - 記憶體控制 IC'),
    "晶達": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "智伸科": ('車用與傳統工業', '汽車動力與傳動精密零件'),
    "智原": ('半導體產業', 'IC設計 - 矽智財與 ASIC (IP/ASIC)'),
    "智基": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "智微": ('半導體產業', 'IC設計 - 記憶體控制 IC'),
    "智捷": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "智易": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "智晶": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "暉盛": ('通訊、線材與連接器', '光通訊與光模組'),
    "暉盛-創": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "曜越": ('工業電腦與電腦週邊', '散熱模組與元件'),
    "朋億": ('半導體與 PCB 設備/材料', '半導體高純度化學品供應系統/廠務'),
    "朋億*": ('半導體與 PCB 設備/材料', '半導體廠務與設備工程'),
    "朋程": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "李洲": ('光電與顯示面板', 'LED 封裝與代理商'),
    "李洲科技": ('光電與顯示面板', 'LED封裝與代理商'),
    "杏一": ('生技醫療', '醫藥通路與連鎖藥局'),
    "杰力": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "東元": ('電子零組件與其它', '馬達、變壓器與重電設備'),
    "東典光電": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "東台": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "東捷科技": ('半導體與 PCB 設備/材料', '半導體與面板製程設備'),
    "東浦": ('車用與傳統工業', '汽車與消費電子塑膠外殼/模具'),
    "東研信超": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "東碩": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "東訊": ('通訊、線材與連接器', '光通訊與光模組'),
    "東鋼": ('鋼鐵與金屬', '特殊鋼與鋼材加工'),
    "東陽": ('車用與傳統工業', '汽車零組件 - 車體與外觀件'),
    "東鹼": ('綠能、環保與化學工業', '硫酸鉀肥料與散裝航運'),
    "松川精密": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "松普": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "松翰": ('半導體產業', 'IC設計 - 微控制器 (MCU)'),
    "柏承": ('PCB 與銅箔基板', '印刷電路板 (PCB)'),
    "柏騰": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "根基": ('建材營造與房地產', '營建與不動產開發'),
    "桂盟": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "桓達": ('電子零組件與其它', '感測器與自動化控制'),
    "桓鼎-KY": ('建材營造與房地產', '建材與裝修'),
    "楠梓電": ('PCB 與銅箔基板', '印刷電路板 (PCB)'),
    "榮剛": ('鋼鐵與金屬', '特殊鋼與鋼材加工'),
    "榮創": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "榮惠-KY創": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "榮昌": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "榮田": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "榮群": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "樺漢": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "橋椿": ('傳統工業與其它', '廚衛五金'),
    "橙的": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "欣厚-KY": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "欣普羅": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "欣興": ('PCB 與銅箔基板', 'IC 載板 (ABF/BT)'),
    "欣銓科技": ('半導體產業', '半導體封測 - 晶圓測試'),
    "欣陸": ('建材營造與房地產', '營建與不動產開發'),
    "正基": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "正文": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "正新": ('車用與傳統工業', '汽車零組件 - 輪胎與橡膠'),
    "正淩": ('通訊、線材與連接器', '連接器 - 高頻與光學連接器'),
    "正能量智能": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "正道": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "正達": ('光電與顯示面板', '車用/3D玻片玻璃'),
    "氣立": ('電子零組件與其它', '電子零組件 - 機械機構與氣動元件'),
    "永崴投控": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "永彰": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "永擎": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "永新-KY": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "永道-KY": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "江申": ('車用與傳統工業', '汽車零組件 - 傳動與引擎動力零件'),
    "江興鍛": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "沛亨": ('半導體產業', 'IC設計 - 多媒體與影音 SoC'),
    "沛爾生醫-創": ('生技醫療', '細胞療法與免疫製劑'),
    "泓德能源": ('建材營造與房地產', '營建與不動產開發'),
    "泓格科技": ('工業電腦與電腦週邊', '工業乙太網路與遠端監控系統'),
    "波若威": ('通訊、線材與連接器', '光通訊與光模組'),
    "泰茂": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "泰藝": ('被動元件與石英元件', '石英元件與頻率控制元件'),
    "泰谷": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "泰豐": ('車用與傳統工業', '汽車零組件 - 輪胎與橡膠'),
    "泰金寶-DR": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "泰霖": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "洋基工程": ('半導體與 PCB 設備/材料', '半導體廠務與設備工程'),
    "浪凡": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "海德威": ('半導體產業', 'IC設計 - 顯示驅動與觸控 IC'),
    "海柏特": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "海華": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "淳安": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "淳紳": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "湧德": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "源傑": ('通訊、線材與連接器', '光通訊與光模組'),
    "準訊": ('通訊、線材與連接器', '光通訊與光模組'),
    "漢唐": ('半導體與 PCB 設備/材料', '半導體廠務與設備工程'),
    "潤弘": ('建材營造與房地產', '營建與不動產開發'),
    "潤德": ('建材營造與房地產', '空間設計與顧問服務'),
    "潤泰全": ('建材營造與房地產', '營建與不動產開發'),
    "澤米": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "濱川": ('車用與傳統工業', '精密機械與金屬加工 - 鑄造鍛造與合金外殼'),
    "瀚荃": ('通訊、線材與連接器', '連接器 - 高頻與光學連接器'),
    "瀧澤科": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "為升": ('車用與傳統工業', '汽車零組件 - 車用電子與感測器'),
    "燁輝": ('傳統工業與傳統材料', '鋼鐵金屬'),
    "營邦": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "玖鼎電力": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "瑞倡": ('半導體產業', 'IC設計 - 顯示驅動與觸控 IC'),
    "瑞利": ('車用與傳統工業', '汽車零組件 - 車體與外觀件'),
    "瑞昱": ('半導體產業', 'IC設計 - 聯網與射頻晶片'),
    "瑞智": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "瑞祺電通": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "瑞穎": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "瑞築": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "瑞鼎": ('半導體產業', 'IC設計 - 顯示驅動與觸控 IC'),
    "璟德": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "環天科": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "環宇-KY": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "環球晶": ('半導體產業', '矽晶圓材料'),
    "環科": ('電子零組件與其它', '電子零組件 - 聲學與感測元件'),
    "界霖": ('半導體產業', '半導體導線架 (封裝材料)'),
    "百一": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "百容": ('半導體產業', '機電元件 (開關/繼電器) 與半導體導線架'),
    "百德": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "百徽": ('半導體產業', '電子零組件代理商'),
    "百達-KY": ('車用與傳統工業', '汽車精密沖壓與服務機器人'),
    "皇將": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "皇昌": ('建材營造與房地產', '營建與不動產開發'),
    "皇田": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "盈正": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "盛群": ('半導體產業', 'IC設計 - 微控制器 (MCU)'),
    "盟立": ('半導體與 PCB 設備/材料', '自動化倉儲與機器人系統整合'),
    "直得": ('半導體產業', 'IC設計 - AI 加速器與邊緣運算晶片'),
    "眾福科": ('被動元件與石英元件', '石英元件與頻率控制元件'),
    "眾達-KY": ('通訊、線材與連接器', '光通訊與光模組'),
    "矽創": ('半導體產業', 'IC設計 - 顯示驅動與觸控 IC'),
    "矽力*-KY": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "矽格": ('半導體產業', '半導體封測 - 射頻/混合訊號/測試'),
    "矽瑪": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "研揚": ('工業電腦與電腦週邊', '工業電腦 (IPC)'),
    "研華": ('工業電腦與電腦週邊', '工業電腦 (IPC)'),
    "研通": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "碩天": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "碩天科技": ('電子零組件與其它', '電源供應器與適配器'),
    "磁震": ('光電與顯示面板', '光電與顯示零組件與材料'),
    "磐亞": ('綠能、環保與化學工業', '非離子界面活性劑原料'),
    "磐儀": ('工業電腦與電腦週邊', '工業電腦 (IPC)'),
    "神基": ('工業電腦與電腦週邊', '強固型電腦與車用機構件'),
    "神數": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "神準": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "神盾": ('半導體產業', 'IC設計 - 影像感測與光學 IC'),
    "神達": ('工業電腦與電腦週邊', '電腦主機與伺服器系統'),
    "祥碩": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "福懋": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "福懋科": ('半導體產業', '半導體封測 - 記憶體模組封裝與測試'),
    "福興": ('傳統工業與其它', '鎖具與五金件'),
    "福華": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "福裕": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "禾伸堂": ('被動元件與石英元件', '電阻/電容/MLCC'),
    "禾昌": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "禾瑞亞": ('半導體產業', 'IC設計 - 顯示驅動與觸控 IC'),
    "科嘉-KY": ('工業電腦與電腦週邊', '電腦周邊 - 鍵盤與輸入裝置'),
    "科嶠": ('半導體與 PCB 設備/材料', 'PCB 乾燥製程設備'),
    "科風": ('電子零組件與其它', '電源供應器'),
    "程泰": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "穎漢": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "穩懋": ('通訊、線材與連接器', '光通訊與光模組'),
    "立凱-KY": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "立敦": ('被動元件與石英元件', '電解電容與薄膜電容'),
    "立敦科技": ('被動元件與石英元件', '化成箔 (電解電容材料)'),
    "立積": ('半導體產業', 'IC設計 - 聯網與射頻晶片'),
    "立端": ('工業電腦與電腦週邊', '工業電腦 (IPC)'),
    "立衛": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "立隆電": ('被動元件與石英元件', '電解電容與薄膜電容'),
    "竹陞科技": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "笙泉": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "笙科": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "精元": ('工業電腦與電腦週邊', '電腦周邊 - 鍵盤與輸入裝置'),
    "精剛": ('車用與傳統工業', '精密機械與金屬加工 - 鑄造鍛造與合金外殼'),
    "精技": ('工業電腦與電腦週邊', '電腦周邊 - 印表機與條碼設備'),
    "精拓科": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "精材": ('半導體產業', '半導體封測 - 晶圓級封裝'),
    "精確": ('車用與傳統工業', '精密機械與金屬加工 - 零組件加工'),
    "精聯": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "系統電": ('車用與傳統工業', '車用電子與感測器'),
    "統一證": ('金融服務', '證券公司'),
    "統懋": ('半導體產業', '功率半導體二極體'),
    "統新": ('通訊、線材與連接器', '光通訊與光模組'),
    "維熹": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "維田": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "緯創": ('伺服器與資訊週邊', '伺服器與筆電代工'),
    "緯穎": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "美吉吉-KY": ('建材營造與房地產', '建材與裝修'),
    "美好證": ('金融服務', '證券公司'),
    "美而快": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "群光": ('工業電腦與電腦週邊', '電腦周邊 - 鍵盤與輸入裝置'),
    "群創": ('光電與顯示面板', '顯示面板與液晶螢幕'),
    "群聯": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "群電": ('電子零組件與其它', '電源供應器與適配器'),
    "義隆": ('半導體產業', 'IC設計 - 微控制器 (MCU)'),
    "翔名科技": ('半導體與 PCB 設備/材料', '半導體離子植入機耗材'),
    "翔耀": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "耀登": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "耐德": ('紡織與成衣', '機能性布料與成衣 OEM'),
    "耿鼎": ('車用與傳統工業', '汽車零組件 - 車體與外觀件'),
    "聖暉": ('半導體與 PCB 設備/材料', '無塵室及機電工程'),
    "聖暉*": ('半導體與 PCB 設備/材料', '半導體廠務與設備工程'),
    "聚亨": ('車用與傳統工業', '工具與金屬配件 - 螺絲螺帽與緊固件'),
    "聚積": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "聚陽": ('紡織與成衣', '機能性布料與成衣 OEM'),
    "聯上": ('建材營造與房地產', '營建與不動產開發'),
    "聯亞": ('通訊、線材與連接器', '光通訊與光模組'),
    "聯傑": ('半導體產業', 'IC設計 - 聯網與射頻晶片'),
    "聯光通": ('通訊、線材與連接器', '光通訊與光模組'),
    "聯友金屬-創": ('綠能、環保與化學工業', '鎢鈷回收與特用金屬化學'),
    "聯嘉投控": ('電子零組件與其它', '車用與特殊照明模組'),
    "聯德控股-KY": ('車用與傳統工業', '精密機械與金屬加工 - 零組件加工'),
    "聯成": ('綠能、環保與化學工業', '石化原料與特用化學品'),
    "聯發": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "聯發科": ('半導體產業', 'IC設計 - 多媒體與影音 SoC'),
    "聯穎": ('通訊、線材與連接器', '光通訊與光模組'),
    "聯茂": ('PCB 與銅箔基板', '銅箔基板 (CCL)'),
    "聯詠": ('半導體產業', 'IC設計 - 顯示驅動與觸控 IC'),
    "聯鈞": ('通訊、線材與連接器', '光通訊與光模組'),
    "聯電": ('半導體產業', '晶圓代工'),
    "聰泰": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "胡連": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "能率": ('工業電腦與電腦週邊', '辦公與商業設備'),
    "能率亞洲": ('車用與傳統工業', '創業投資與資產管理'),
    "能率網通": ('工業電腦與電腦週邊', '通訊代理與通路服務'),
    "至興": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "致和證": ('金融服務', '證券公司'),
    "致新": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "致茂": ('半導體與 PCB 設備/材料', '精密測試設備'),
    "臺慶科": ('被動元件與石英元件', '電感器與磁性元件'),
    "臻鼎-KY": ('PCB 與銅箔基板', '硬板、軟板與載板全方位板王'),
    "興勤": ('被動元件與石英元件', '保護元件與過電壓保護'),
    "興富發": ('建材營造與房地產', '營建與不動產開發'),
    "興能高": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "良維": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "艾姆勒": ('車用與傳統工業', '汽車零組件 - 傳動與引擎動力零件'),
    "艾恩特": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "艾訊": ('工業電腦與電腦週邊', '工業電腦 (IPC)'),
    "芯鼎": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "英利-KY": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "英業達": ('工業電腦與電腦週邊', '電腦主機與伺服器系統'),
    "茂矽": ('半導體產業', '晶圓代工/二極體'),
    "茂訊": ('工業電腦與電腦週邊', '電腦主機與伺服器系統'),
    "茂達": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "茂達電子": ('半導體產業', 'IC設計 - 電源管理 IC'),
    "茂順": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "華冠": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "華勝-KY": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "華孚": ('車用與傳統工業', '精密機械與金屬加工 - 鑄造鍛造與合金外殼'),
    "華容": ('被動元件與石英元件', '電解電容與薄膜電容'),
    "華建": ('建材營造與房地產', '營建與不動產開發'),
    "華擎": ('工業電腦與電腦週邊', '品牌電腦與主機板/顯示卡'),
    "華新科": ('被動元件與石英元件', '電阻/電容/MLCC'),
    "華星光": ('通訊、線材與連接器', '光通訊與光模組'),
    "華景電": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "華晶科": ('光電與顯示面板', '數位相機與影像處理'),
    "華洋精機": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "華盈": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "華碩": ('工業電腦與電腦週邊', '品牌電腦與主機板/顯示卡'),
    "華祺": ('車用與傳統工業', '工具與金屬配件 - 螺絲螺帽與緊固件'),
    "華立": ('半導體與 PCB 設備/材料', '半導體設備與材料'),
    "華紡": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "華興": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "華豐": ('車用與傳統工業', '汽車零組件 - 輪胎與橡膠'),
    "華豫寧": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "華通": ('PCB 與銅箔基板', '高密度連接板 (HDI) 與軟硬結合板'),
    "華邦電": ('半導體產業', '記憶體製造 (DRAM/Flash)'),
    "華電": ('綠能、環保與化學工業', '重電與電線電纜'),
    "菱光": ('通訊、線材與連接器', '光通訊與光模組'),
    "菱生": ('半導體產業', '半導體封測 - 電源管理/感測器封測'),
    "萊德光電-KY": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "萬在": ('車用與傳統工業', '汽車零組件 - 車體與外觀件'),
    "萬旭": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "萬泰科": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "萬泰科技": ('通訊、線材與連接器', '高頻網路線與同軸線'),
    "萬潤": ('半導體與 PCB 設備/材料', '半導體設備與材料'),
    "蔚華科": ('半導體與 PCB 設備/材料', '半導體測試設備與代理'),
    "藍天": ('工業電腦與電腦週邊', '品牌電腦與主機板/顯示卡'),
    "藥華藥": ('生技醫療', '罕見疾病新藥研發'),
    "虎山": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "虹光": ('工業電腦與電腦週邊', '電腦周邊 - 掃描器與影像擷取'),
    "虹揚-KY": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "蜜望實": ('被動元件與石英元件', '被動元件通路商'),
    "融程電": ('工業電腦與電腦週邊', '工業電腦 (IPC)'),
    "西勝": ('電子零組件與其它', '電池與儲能系統'),
    "訊舟": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "詠昇": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "詮欣": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "誠研": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "謙達": ('半導體產業', 'IC設計 - AI 加速器與邊緣運算晶片'),
    "謚源": ('車用與傳統工業', '汽車零組件 - 車用電子與感測器'),
    "譁裕": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "譜瑞-KY": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "谷崧": ('通訊、線材與連接器', '光通訊與光模組'),
    "豐達科": ('車用與傳統工業', '工具與金屬配件 - 螺絲螺帽與緊固件'),
    "豪勉": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "貿聯-KY": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "超豐": ('半導體產業', '半導體封測 - 導線架打線封測'),
    "越峰": ('被動元件與石英元件', '電感器與磁性元件'),
    "越峰電子": ('被動元件與石英元件', '電感鐵芯/錳鋅及鎳鋅鐵氧體電磁材料'),
    "車王電": ('車用與傳統工業', '汽車零組件 - 車用電子與感測器'),
    "輝創": ('車用與傳統工業', '汽車零組件 - 車用電子與感測器'),
    "辛耘": ('半導體與 PCB 設備/材料', '半導體設備與材料'),
    "農林": ('紡織與成衣', '紡織、紗線與布料'),
    "迅德": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "迅杰": ('半導體產業', 'IC設計 - 微控制器 (MCU)'),
    "迎廣": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "通嘉": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "通泰": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "連展投控": ('通訊、線材與連接器', '連接器 - 車用與工業連接器'),
    "進典": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "逸昌": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "達方": ('工業電腦與電腦週邊', '電腦周邊 - 鍵盤與輸入裝置'),
    "達明": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "達欣工": ('建材營造與房地產', '營建與不動產開發'),
    "達發": ('半導體產業', 'IC設計 - 聯網與射頻晶片'),
    "達盛電": ('半導體產業', 'IC設計 - 記憶體控制 IC'),
    "達航科技": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "達輝-KY": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
    "達運": ('通訊、線材與連接器', '網通 - 企業交換器與企業網路'),
    "達運光電": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "遠東新": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "遠雄": ('建材營造與房地產', '營建與不動產開發'),
    "邁科": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "邑昇": ('通訊、線材與連接器', '光通訊與光模組'),
    "邑錡": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "采鈺": ('半導體產業', 'IC設計 - 影像感測與光學 IC'),
    "金像電": ('PCB 與銅箔基板', '高階伺服器板 (PCB)'),
    "金寶": ('工業電腦與電腦週邊', '消費性電子、通訊設備代工'),
    "金山電": ('被動元件與石英元件', '電阻/電容/MLCC'),
    "金山電子": ('被動元件與石英元件', '電解電容'),
    "金橋": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "金興精密": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "金麗科": ('半導體產業', 'IC設計 - 矽智財與 ASIC (IP/ASIC)'),
    "鈞寶": ('被動元件與石英元件', '電感器與磁性元件'),
    "鈞興-KY": ('車用與傳統工業', '工具與金屬配件 - 傳動元件'),
    "鈦昇": ('半導體與 PCB 設備/材料', '半導體雷射切割與修整設備'),
    "鈺創": ('半導體產業', 'IC設計 - 記憶體控制 IC'),
    "鈺創科技": ('半導體產業', 'IC設計 - 記憶體 IC 與邏輯晶片'),
    "鈺太": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "鈺寶-創": ('半導體產業', 'IC設計 - 聯網與射頻晶片'),
    "鈺邦": ('被動元件與石英元件', '電解電容與薄膜電容'),
    "鉅晶電子": ('半導體產業', 'IC設計 - 矽智財與 ASIC (IP/ASIC)'),
    "鉅祥": ('半導體與 PCB 設備/材料', '精密衝壓與精密金屬組件'),
    "銘異": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "銘鈺": ('車用與傳統工業', '精密機械與金屬加工 - 零組件加工'),
    "鋐寶科技": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "錩新": ('車用與傳統工業', '精密機械與金屬加工 - 鑄造鍛造與合金外殼'),
    "錩泰": ('車用與傳統工業', '工具與金屬配件 - 手工具與氣動工具'),
    "錸德": ('被動元件與石英元件', '石英元件與頻率控制元件'),
    "鎰勝": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "鐿鈦": ('車用與傳統工業', '工具與金屬配件 - 手工具與氣動工具'),
    "鑫創": ('半導體產業', 'IC設計 - 記憶體控制 IC'),
    "鑫創電子": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "鑫永洋": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "鑫永銓": ('車用與傳統工業', '汽車零組件 - 輪胎與橡膠'),
    "鑫禾": ('車用與傳統工業', '工具與金屬配件 - 其它五金製品'),
    "鑫科": ('車用與傳統工業', '精密機械與金屬加工 - 轉軸與機構件'),
    "鑽全": ('車用與傳統工業', '工具與金屬配件 - 手工具與氣動工具'),
    "鑽石投資": ('金融服務', '創業投資與資產管理'),
    "長佳": ('建材營造與房地產', '營建與不動產開發'),
    "長園科": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "長榮航太": ('軍工與航太', '飛機維修與航太零組件製造'),
    "長盛": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "長科": ('半導體產業', '導線架材料 (封裝材料)'),
    "長興": ('綠能、環保與化學工業', '合成樹脂與特用化學品'),
    "長華": ('半導體與 PCB 設備/材料', '半導體設備與材料'),
    "閎康": ('半導體產業', 'IC 研發與分析服務 (MA/FA)'),
    "閎暉": ('電子零組件與其它', '電子零組件 - 機械機構與氣動元件'),
    "陽程": ('半導體與 PCB 設備/材料', '自動化物流與貼合設備'),
    "隆大": ('建材營造與房地產', '營建與不動產開發'),
    "隴華": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "集盛": ('紡織與成衣', '紡紗、化纖與紡織原料'),
    "雙喜": ('建材營造與房地產', '營建與不動產開發'),
    "雙鍵": ('綠能、環保與化學工業', '特用化學品 (塑膠添加劑/塗料)'),
    "零壹": ('通訊、線材與連接器', '網通 - 低軌衛星與太空通信'),
    "雷科": ('被動元件與石英元件', '被動元件上游材料與設備'),
    "雷虎": ('軍工與航太', '無人機與遙控模型'),
    "霖宏科技": ('PCB 與銅箔基板', '多層印刷電路板'),
    "青鋼": ('建材營造與房地產', '建材與裝修'),
    "青雲": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "韋僑": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "順德": ('半導體與 PCB 設備/材料', '功率半導體導線架與模具'),
    "順達": ('電子零組件與其它', '電池與儲能系統'),
    "類比科": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "風青": ('通訊、線材與連接器', '漆包線 (繞線用)'),
    "飛宏": ('電子零組件與其它', '電源供應器'),
    "飛捷": ('工業電腦與電腦週邊', 'POS 與自助服務終端'),
    "首利": ('電子零組件與其它', '電源供應器與適配器'),
    "馥鴻": ('電子零組件與其它', '電子零組件 - 安防與控制系統'),
    "騰輝電子-KY": ('PCB 與銅箔基板', '銅箔基板 (CCL)'),
    "驊宏資": ('通訊、線材與連接器', '網通 - 寬頻 CPE 與 Wi-Fi 路由器'),
    "驊訊": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "驊陞": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "高僑": ('車用與傳統工業', '工具與金屬配件 - 傳動元件'),
    "高林股": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "高鋒": ('半導體與 PCB 設備/材料', '工具機與精密加工機械'),
    "鴻名": ('工業電腦與電腦週邊', '電腦主機與伺服器系統'),
    "鴻呈": ('通訊、線材與連接器', '連接器 - 電子與消費性線材'),
    "鴻海": ('工業電腦與電腦週邊', '全方位電子代工 (EMS) 龍頭'),
    "鴻準": ('車用與傳統工業', '機殼與沖壓件'),
    "鴻碩": ('電子零組件與其它', '電氣設備與零件 - 工業控制與配電器材'),
    "鴻騰精密": ('通訊、線材與連接器', '光通訊與光模組'),
    "麗正": ('半導體產業', 'IC設計 - 類比與電源管理'),
    "麗清": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "麗臺": ('工業電腦與電腦週邊', '品牌電腦與主機板/顯示卡'),
    "點序": ('工業電腦與電腦週邊', '電腦周邊 - 其它週邊與耗材'),
    "點晶": ('半導體產業', 'IC設計 - 消費性與其它 IC'),
    "鼎天": ('車用與傳統工業', '車用電子與導航系統'),
    "鼎炫-KY": ('電子零組件與其它', '電子零組件 - 其它電子組件'),
    "鼎翰": ('工業電腦與電腦週邊', '電腦周邊 - 印表機與條碼設備'),
    "龍邦": ('建材營造與房地產', '營建與不動產開發'),
    "龍鋒": ('車用與傳統工業', '汽車零組件 - 車內裝與其他五金'),
}

def clean_stock_name(name):
    n = name.replace("*", "").replace("-KY", "").replace("-創", "").replace("（", "").replace("）", "").replace("(", "").replace(")", "")
    n = n.replace("台灣", "").replace("台", "")
    n = re.sub(r'(精密|科技|電子|電機|電陶|光電|工業|控股|國際|生醫|生物科技|金屬|服務|通路|網通|亞洲)', '', n)
    return n

CLEAN_SUBCLASS = {clean_stock_name(k): v for k, v in STOCK_SUBCLASS.items()}

DETAILED_SECTOR_MAP = {
    "Advertising Agencies": ("軟體與資訊服務", "數位廣告與行銷科技"),
    "Building Products & Equipment": ("建材營造與房地產", "建材與裝修"),
    "Consulting Services": ("建材營造與房地產", "空間設計與顧問服務"),
    "Pharmaceutical Retailers": ("生技醫療", "醫藥通路與連鎖藥局"),
    "Scientific & Technical Instruments": ("半導體與 PCB 設備/材料", "量測儀器與精密檢測"),
    "N/A": ("金融服務", "ETF與基金"),
    # Technology
    "Semiconductors": ("半導體產業", "半導體製造與設備"),
    "Semiconductor Equipment & Materials": ("半導體產業", "半導體設備與材料"),
    "Electronic Components": ("電子零組件與其它", "電子零組件"),
    "Electronic Distribution": ("半導體與電子通路", "電子通路與經銷"),
    "Electronics & Computer Distribution": ("半導體與電子通路", "電子通路與經銷"),
    "Computer Hardware": ("工業電腦與電腦週邊", "電腦及週邊設備"),
    "Consumer Electronics": ("工業電腦與電腦週邊", "消費性電子"),
    "Communication Equipment": ("通訊、線材與連接器", "通信與網路設備"),
    "Software - Application": ("軟體與資訊服務", "資訊服務與軟體外包"),
    "Software-Application": ("軟體與資訊服務", "資訊服務與軟體外包"),
    "Software - Infrastructure": ("軟體與資訊服務", "數位雲端服務"),
    "Software-Infrastructure": ("軟體與資訊服務", "數位雲端服務"),
    "Information Technology Services": ("軟體與資訊服務", "資訊服務與軟體外包"),
    "Internet Content & Information": ("軟體與資訊服務", "數位雲端服務"),
    "Electronic Gaming & Multimedia": ("軟體與資訊服務", "數位娛樂與多媒體"),
    
    # Industrials & Manufacturing
    "Aerospace & Defense": ("軍工與航太", "軍工與航太零組件"),
    "Pollution & Treatment Controls": ("綠能、環保與化學工業", "環保綠能與水處理"),
    "Specialty Industrial Machinery": ("半導體與 PCB 設備/材料", "特殊及精密工業機械"),
    "Electrical Equipment & Parts": ("電子零組件與其它", "電氣設備與零件"),
    "Metal Fabrication": ("車用與傳統工業", "精密機械與金屬加工"),
    "Tools & Accessories": ("車用與傳統工業", "工具與金屬配件"),
    "Industrial Distribution": ("車用與傳統工業", "工業材料通路"),
    "Engineering & Construction": ("半導體與 PCB 設備/材料", "廠務與工程建設"),
    "Business Equipment & Supplies": ("工業電腦與電腦週邊", "辦公與商業設備"),
    "Specialty Business Services": ("車用與傳統工業", "商業與外包服務"),
    "Staffing & Employment Services": ("軟體與資訊服務", "人力資源與資訊外包"),
    "Waste Management": ("綠能、環保與化學工業", "廢棄物處理"),
    "Conglomerates": ("傳統工業與其它", "綜合企業控股"),

    # Auto & Vehicles
    "Auto Parts": ("車用與傳統工業", "汽車零組件"),
    "Auto Manufacturers": ("車用與傳統工業", "汽車製造"),
    "Auto & Truck Dealerships": ("車用與傳統工業", "汽機車經銷"),
    "Recreational Vehicles": ("車用與傳統工業", "休閒車輛"),
    
    # Chemicals & Materials
    "Specialty Chemicals": ("綠能、環保與化學工業", "特用化學品"),
    "Chemicals": ("綠能、環保與化學工業", "化學工業"),
    "Agricultural Inputs": ("綠能、環保與化學工業", "農業與肥料"),
    "Steel": ("傳統工業與傳統材料", "鋼鐵金屬"),
    "Aluminum": ("傳統工業與傳統材料", "鋁業金屬"),
    "Other Industrial Metals & Mining": ("傳統工業與傳統材料", "其它金屬礦產"),
    "Copper": ("傳統工業與傳統材料", "銅業"),
    "Lumber & Wood Production": ("傳統工業與傳統材料", "木材與造紙上游"),
    "Paper & Paper Products": ("傳統工業與傳統材料", "造紙"),
    "Packaging & Containers": ("傳統工業與傳統材料", "包裝與容器"),
    "Building Materials": ("傳統工業與傳統材料", "建材"),

    # Consumer Defensive
    "Beverages - Non-Alcoholic": ("傳統工業與其它", "飲料與非酒精飲料"),
    "Beverages-Non-Alcoholic": ("傳統工業與傳統材料", "食品與飲料"),
    "Packaged Foods": ("傳統工業與其它", "包裝食品"),
    "Food Distribution": ("傳統工業與其它", "食品通路"),
    "Confectioners": ("傳統工業與其它", "食品與糖果"),
    "Farm Products": ("傳統工業與其它", "農產品"),
    "Grocery Stores": ("傳統工業與其它", "超市與量販"),
    "Household & Personal Products": ("傳統工業與其它", "民生與個人清潔用品"),

    # Consumer Cyclical & Retail
    "Luxury Goods": ("傳統工業與其它", "精品珠寶"),
    "Specialty Retail": ("傳統工業與其它", "百貨貿易與零售"),
    "Apparel Retail": ("傳統工業與其它", "服飾零售"),
    "Department Stores": ("傳統工業與其它", "百貨公司"),
    "Internet Retail": ("傳統工業與其它", "電子商務"),
    "Home Improvement Retail": ("傳統工業與其它", "居家生活零售"),
    "Travel Services": ("觀光餐旅與休閒娛樂", "觀光旅遊與票務"),
    "Lodging": ("觀光餐旅與休閒娛樂", "觀光餐旅 - 飯店"),
    "Restaurants": ("觀光餐旅與休閒娛樂", "觀光餐旅 - 餐飲"),
    "Resorts & Casinos": ("觀光餐旅與休閒娛樂", "娛樂與休閒"),
    "Footwear & Accessories": ("傳統工業與其它", "鞋類與配飾"),
    "Furnishings, Fixtures & Appliances": ("傳統工業與其它", "家具與家電"),
    "Apparel Manufacturing": ("傳統工業與傳統材料", "成衣製造"),
    "Textile Manufacturing": ("傳統工業與傳統材料", "紡織纖維"),
    
    # Financials
    "Banks - Regional": ("金融服務", "銀行與金融控股"),
    "Banks-Regional": ("金融服務", "銀行與金融控股"),
    "Capital Markets": ("金融服務", "證券與資本市場"),
    "Asset Management": ("金融服務", "創業投資與資產管理"),
    "Credit Services": ("金融服務", "信用貸款與消費金融"),
    "Financial Conglomerates": ("金融服務", "金融控股公司"),
    "Insurance - Diversified": ("金融服務", "保險業"),
    "Insurance - Life": ("金融服務", "人壽保險"),
    "Insurance - Property & Casualty": ("金融服務", "產物與意外險"),
    "Insurance - Reinsurance": ("金融服務", "再保險"),
    "Insurance Brokers": ("金融服務", "保險經紀人"),

    # Healthcare
    "Biotechnology": ("生技醫療", "生物科技"),
    "Drug Manufacturers - Specialty & Generic": ("生技醫療", "醫療與生技製藥"),
    "Drug Manufacturers-Specialty & Generic": ("生技醫療", "醫療與生技製藥"),
    "Drug Manufacturers - General": ("生技醫療", "醫療與生技製藥"),
    "Drug Manufacturers-General": ("生技醫療", "醫療與生技製藥"),
    "Diagnostics & Research": ("生技醫療", "醫療檢測與研究"),
    "Medical Instruments & Supplies": ("生技醫療", "醫療器材與耗材"),
    "Medical Devices": ("生技醫療", "醫療器材與設備"),
    "Medical Distribution": ("生技醫療", "醫療通路與醫藥分銷"),
    "Medical Care Facilities": ("生技醫療", "醫療照護機構"),
    "Health Information Services": ("生技醫療", "醫療資訊服務"),

    # Real Estate & Utilities & Telecom
    "Real Estate - Development": ("建材營造與房地產", "營建與不動產開發"),
    "Real Estate-Development": ("建材營造與房地產", "營建與不動產開發"),
    "Real Estate - Diversified": ("建材營造與房地產", "不動產綜合"),
    "Real Estate-Diversified": ("建材營造與房地產", "不動產綜合"),
    "Real Estate Services": ("建材營造與房地產", "不動產中介與服務"),
    "Homebuilding": ("建材營造與房地產", "住宅建造"),
    
    "Utilities - Regulated Electric": ("綠能、環保與化學工業", "電力能源公用事業"),
    "Utilities-Regulated Electric": ("綠能、環保與化學工業", "電力能源公用事業"),
    "Utilities - Regulated Gas": ("綠能、環保與化學工業", "油氣公用事業"),
    "Utilities-Regulated Gas": ("綠能、環保與化學工業", "油氣公用事業"),
    "Utilities - Regulated Water": ("綠能、環保與化學工業", "水力公用事業"),
    "Utilities-Regulated Water": ("綠能、環保與化學工業", "水力公用事業"),
    "Utilities - Renewable": ("綠能、環保與化學工業", "綠能環保與再生能源"),
    "Utilities-Renewable": ("綠能、環保與化學工業", "綠能環保與再生能源"),
    "Solar": ("綠能、環保與化學工業", "太陽能綠能"),
    "Thermal Coal": ("綠能、環保與化學工業", "煤炭與傳統能源"),

    "Telecom Services": ("通訊、線材與連接器", "電信與通訊服務"),
    "Broadcasting": ("觀光餐旅與休閒娛樂", "傳播與媒體"),
    "Publishing": ("觀光餐旅與休閒娛樂", "出版與數位媒體"),
    "Entertainment": ("觀光餐旅與休閒娛樂", "娛樂與文創"),
    "Personal Services": ("傳統工業與其它", "生活服務"),
    "Education & Training Services": ("觀光餐旅與休閒娛樂", "教育與培訓"),
    "Leisure": ("觀光餐旅與休閒娛樂", "運動與休閒娛樂"),
    "Security & Protection Services": ("軟體與資訊服務", "資訊安全與保全"),
    
    # Logistics & Transport
    "Integrated Freight & Logistics": ("航運與運輸", "物流與倉儲"),
    "Marine Shipping": ("航運與運輸", "航運 - 散裝/貨櫃"),
    "Airlines": ("航運與運輸", "航空公司"),
    "Railroads": ("航運與運輸", "陸運鐵路"),
    "Trucking": ("航運與運輸", "陸運與物流"),
    "Oil & Gas Refining & Marketing": ("綠能、環保與化學工業", "石油煉製與行銷"),
    "Oil & Gas Equipment & Services": ("綠能、環保與化學工業", "石油與天然氣設備服務"),
    
    # TWSE/TPEx Chinese Fallbacks
    "半導體業": ("半導體產業", "半導體製造與設備"),
    "電腦及週邊設備業": ("工業電腦與電腦週邊", "電腦及週邊設備"),
    "光電業": ("光電與顯示面板", "光電與顯示面板"),
    "化學工業": ("綠能、環保與化學工業", "化學工業"),
    "橡膠工業": ("傳統工業與傳統材料", "橡膠製品"),
    "汽車工業": ("傳統工業與其它", "汽車與零組件"),
    "貿易百貨業": ("傳統工業與其它", "百貨貿易"),
    "觀光餐旅": ("傳統工業與其它", "觀光旅遊與餐飲"),
    "農業科技業": ("傳統工業與其它", "農業與精緻科技"),
    "文化創意業": ("傳統工業與其它", "文創與傳媒"),
    "運動休閒": ("傳統工業與其它", "運動休閒"),
    "居家生活": ("傳統工業與其它", "居家生活"),
    "其他業": ("傳統工業與其它", "其他未分類傳統行業"),
    "綜合": ("傳統工業與其它", "綜合控股"),
    "其他電子業": ("電子零組件與其它", "其它電子組件"),
    "電子工業": ("電子零組件與其它", "電子製造"),
    "臺灣存託憑證(DR)": ("傳統工業與其它", "存託憑證(DR)"),
}

def load_stock_list(filepath):
    """Loads all tickers and names from stocks_list.txt."""
    stocks = []
    if not os.path.exists(filepath):
        print(f"Error: {filepath} not found.")
        return []
    with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            parts = line.split(",", 1)
            if len(parts) >= 2:
                ticker = parts[0].strip().lstrip('\ufeff')
                name = parts[1].strip()
                stocks.append((name, ticker))
    return stocks

def fetch_yfinance_details(name, ticker, cache):
    """Fetches details from yfinance or cache."""
    if ticker in cache:
        return cache[ticker]
    try:
        t_obj = yf.Ticker(ticker)
        info = t_obj.info
        sector = info.get("sector")
        industry = info.get("industry")
        res = {
            "name": name,
            "ticker": ticker,
            "sector": sector if sector else "N/A",
            "industry": industry if industry else "N/A",
            "longName": info.get("longName", name),
            "status": "success"
        }
        return res
    except Exception as e:
        return {
            "name": name,
            "ticker": ticker,
            "sector": "N/A",
            "industry": "N/A",
            "status": "error",
            "error": str(e)
        }

def save_cache(cache):
    try:
        with open(CACHE_FILE, "w", encoding="utf-8") as f:
            json.dump(cache, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print("Warning: Failed to save cache:", e)

def run_pipeline():
    print("=" * 60)
    print("🚀 Starting Master Database Industry Classification")
    print("=" * 60)
    
    # 1. Load Cache
    cache = {}
    if os.path.exists(CACHE_FILE):
        try:
            with open(CACHE_FILE, "r", encoding="utf-8") as f:
                cache = json.load(f)
            print(f"Loaded {len(cache)} cached stock profiles.")
        except Exception as e:
            print("Warning: Failed to load cache:", e)
            
    # 2. Load stock list
    stocks = load_stock_list(STOCK_LIST_FILE)
    if not stocks:
        print(f"Please place a valid '{STOCK_LIST_FILE}' in the directory.")
        return
    print(f"Loaded {len(stocks)} stocks from database.")
    
    # 3. Find missing tickers
    to_query = []
    details = {}
    for name, ticker in stocks:
        if ticker in cache:
            details[name] = cache[ticker]
            # Ensure name is synced
            details[name]["name"] = name
        else:
            to_query.append((name, ticker))
            
    print(f"Already cached: {len(details)}. Need to query: {len(to_query)}")
    
    # 4. Fetch missing profiles in parallel
    if to_query:
        print(f"Querying {len(to_query)} stock profiles in parallel...")
        batch_size = 50
        count = 0
        
        # We query in small batches and save cache to prevent loss
        for i in range(0, len(to_query), batch_size):
            batch = to_query[i:i+batch_size]
            with ThreadPoolExecutor(max_workers=30) as executor:
                futures = {executor.submit(fetch_yfinance_details, name, ticker, cache): name for name, ticker in batch}
                for fut in as_completed(futures):
                    res = fut.result()
                    name = res["name"]
                    ticker = res["ticker"]
                    details[name] = res
                    if res.get("status") == "success":
                        cache[ticker] = res
            
            count += len(batch)
            save_cache(cache)
            print(f"Progress: {count}/{len(to_query)} queried and cached.")
            
    # 5. Expert Categorization
    print("Categorizing all database stocks...")
    grouped = {}
    
    for name, data in details.items():
        ticker = data.get("ticker", "N/A")
        clean_name = clean_stock_name(name)
        raw_industry = data.get("industry", "Other")
        longName = data.get("longName", "")
        search_text = (name + " " + longName).upper()
        ticker_num = ticker.split(".")[0].strip()
        summary = data.get("longBusinessSummary", "").lower()
        
        def has_kw(kws, in_summary=True):
            target = summary if in_summary else search_text
            for w in kws:
                w_lower = w.lower()
                if len(w_lower) <= 3:
                    if re.search(r'\b' + re.escape(w_lower) + r'\b', target):
                        return True
                else:
                    if w_lower in target:
                        return True
            return False

        is_tech_sector = raw_industry in [
            "Semiconductors", "Semiconductor Equipment & Materials", 
            "Electronic Components", "Computer Hardware", 
            "Communication Equipment", "Software - Application", 
            "Software-Application", "Software - Infrastructure", 
            "Software-Infrastructure", "Information Technology Services", 
            "Electronic Distribution", "Electronics & Computer Distribution"
        ]
        is_industrial_sector = raw_industry in [
            "Electrical Equipment & Parts", "Specialty Industrial Machinery", 
            "Aerospace & Defense", "Engineering & Construction", 
            "Metal Fabrication", "Tools & Accessories"
        ]

        # Determine categories
        if name in STOCK_SUBCLASS:
            main_cat, sub_cat = STOCK_SUBCLASS[name]
        elif clean_name in CLEAN_SUBCLASS:
            main_cat, sub_cat = CLEAN_SUBCLASS[clean_name]
        else:
            # Programmatic ticker cleanups for crowded Electronic Components category
            if ticker_num in ["3023", "3217", "3322", "3533", "3605", "6205", "6272", "8103", "3492", "3511", "3520", "3526", "3597", "3646", "3689", "3710", "3011", "2460", "6126", "6158", "6185", "6418", "5457", "8147", "6913", "3310", "4943"]:
                main_cat, sub_cat = ("通訊、線材與連接器", "連接器與連接線")
            elif ticker_num in ["3390", "3321", "6153", "6269", "3354"]:
                main_cat, sub_cat = ("PCB 與銅箔基板", "軟性銅箔基板 (FCCL)/軟板")
            elif ticker_num in ["3715", "4927", "5469", "6108", "6191", "8213", "3276", "3645", "5291", "5355", "5439", "6156", "6210", "8155", "8074", "8183", "6278", "6266", "8358", "4909", "6194", "6597", "8291"]:
                main_cat, sub_cat = ("PCB 與銅箔基板", "印刷電路板 (PCB)")
            elif ticker_num in ["3653", "3338", "6230", "6124", "3324", "8996", "6125"]:
                main_cat, sub_cat = ("工業電腦與電腦週邊", "散熱模組與元件")
            elif ticker_num in ["1582", "3548", "3230", "3294", "5243"]:
                main_cat, sub_cat = ("電子零組件與其它", "轉軸與機構五金")
            elif ticker_num in ["3049", "4935", "4960", "6120", "6176", "6456", "3622", "3623", "3666", "8105", "8215", "4933", "5220", "5315", "6246", "4729", "8069", "6899", "3285", "3543", "4942", "6405", "6698", "6854", "6916", "8104", "3523", "6167", "8049", "8111", "8240"]:
                main_cat, sub_cat = ("光電與顯示面板", "面板與觸控模組")
            elif ticker_num in ["3591", "3714", "4956", "5244", "2338"]:
                main_cat, sub_cat = ("光電與顯示面板", "LED與光電元件")
            elif ticker_num in ["6498", "6517", "3630", "5230", "3406", "3019", "3504", "3441", "4976", "6668"]:
                main_cat, sub_cat = ("光電與顯示面板", "光電與光學鏡頭")
            elif ticker_num in ["6203", "6276", "6821", "2431"]:
                main_cat, sub_cat = ("電子零組件與其它", "電源供應器")
            elif ticker_num in ["5228", "6284", "4760", "6224", "6792", "6862", "6204"]:
                main_cat, sub_cat = ("被動元件與石英元件", "被動元件 - 電感與磁性元件")
            elif ticker_num in ["3455", "3563", "3093", "7769", "3490", "3551", "5443", "6261", "6425", "6640", "6735", "6877", "6953", "7751", "7556", "3535", "3402", "6438", "6706", "7795", "3485", "3580", "4568", "6208", "6664", "6217"]:
                main_cat, sub_cat = ("半導體與 PCB 設備/材料", "半導體設備與材料")
            elif ticker_num in ["4749", "1773", "4755", "1785", "5234", "4768", "5434", "8070", "3010", "3444", "3305"]:
                main_cat, sub_cat = ("半導體與 PCB 設備/材料", "半導體材料與特用化學品")
            elif ticker_num in ["6451", "6552"]:
                main_cat, sub_cat = ("半導體產業", "IC 封測 (OSAT)")
            elif ticker_num in ["6830"]:
                main_cat, sub_cat = ("半導體產業", "IC 研發與分析服務 (MA/FA)")
            elif ticker_num in ["6725"]:
                main_cat, sub_cat = ("半導體產業", "分離元件與功率半導體")
            elif ticker_num in ["6756", "6962", "5487", "6732", "6996"]:
                main_cat, sub_cat = ("半導體產業", "IC 設計")
            elif ticker_num in ["3416", "4995", "5474", "6577", "7402"]:
                main_cat, sub_cat = ("工業電腦與電腦週邊", "電腦及週邊設備")
            elif ticker_num in ["5284"]:
                main_cat, sub_cat = ("伺服器與資訊週邊", "伺服器與資訊週邊")
            elif ticker_num in ["6609"]:
                main_cat, sub_cat = ("半導體與 PCB 設備/材料", "特殊及精密工業機械")
            elif ticker_num in ["7744"]:
                main_cat, sub_cat = ("通訊、線材與連接器", "通信與網路設備")
            elif ticker_num in ["2347", "3048", "8112", "3036"]:
                main_cat, sub_cat = ("傳統工業與其它", "電子通路")
            elif ticker_num in ["2480", "3029"]:
                main_cat, sub_cat = ("軟體與資訊服務", "資訊服務與軟體外包")
            elif ticker_num in ["2535"]:
                main_cat, sub_cat = ("建材營造與房地產", "營建與不動產開發")
            elif ticker_num in ["6574", "6703", "4137", "4190", "6523", "1786", "6666", "6658"]:
                main_cat, sub_cat = ("生技醫療", "美容保養與醫美")
                
            # Custom overrides for hot sectors (重電, 光通訊)
            elif ticker_num in ["1519", "1503", "1513", "1514", "2371", "1529", "1618", "1608", "1609", "1612", "1617", "1605", "1615", "1616", "2009", "1504"] or has_kw(["重電", "電線電纜", "變壓器", "配電盤"], in_summary=False) or has_kw(["transformer", "switchgear", "heavy electrical", "power cable", "power distribution"]):
                main_cat, sub_cat = ("綠能、環保與化學工業", "重電與電線電纜")
            elif ticker_num in ["3081", "4979", "3234", "4908", "3163", "4977", "3363", "6426", "6442", "3450"] or has_kw(["光通訊", "光模組", "光收發", "光主動", "光被動"], in_summary=False) or has_kw(["optical communication", "optical transceiver", "fiber optic", "laser diode", "optical module"]):
                main_cat, sub_cat = ("通訊、線材與連接器", "光通訊與光模組")
                
            # Raw industry defaults to clean up general categories before keyword check
            elif raw_industry in ["Electronic Distribution", "Electronics & Computer Distribution"]:
                main_cat, sub_cat = ("傳統工業與其它", "電子通路")
            elif raw_industry == "Communication Equipment":
                main_cat, sub_cat = ("通訊、線材與連接器", "通信與網路設備")
            
            # Non-tech and non-industrial traditional sectors bypass custom tech checks
            elif not is_tech_sector and not is_industrial_sector:
                normalized_ind = raw_industry.replace("X", " - ").replace("\u2013", " - ").replace("\u2014", " - ").strip()
                if normalized_ind in DETAILED_SECTOR_MAP:
                    main_cat, sub_cat = DETAILED_SECTOR_MAP[normalized_ind]
                elif raw_industry in DETAILED_SECTOR_MAP:
                    main_cat, sub_cat = DETAILED_SECTOR_MAP[raw_industry]
                else:
                    main_cat = "其他未分類"
                    sub_cat = raw_industry
            
            # 2. Machinery, Equipment and Engineering Override
            elif raw_industry in ["Specialty Industrial Machinery", "Engineering & Construction", "Tools & Accessories"] and has_kw(["semiconductor", "wafer", "pcb", "printed circuit", "ic"]):
                main_cat, sub_cat = ("半導體與 PCB 設備/材料", "半導體與 PCB 製程設備")

            # 4. Semiconductors
            elif raw_industry == "Semiconductors" or raw_industry == "Semiconductor Equipment & Materials" or has_kw(["semiconductor", "integrated circuit", "wafer", "microchip"]):
                if ticker_num in ["2330", "2303", "5347", "6770"]:
                    main_cat, sub_cat = ("半導體產業", "晶圓代工 (Foundry)")
                elif ticker_num in ["3711", "2449", "6239", "6257", "3265", "3289", "8150", "6147", "3374", "6515", "3581", "3372", "8110", "2369", "6272", "3008"] or has_kw(["packaging", "semiconductor testing", "osat", "assembly service"]):
                    main_cat, sub_cat = ("半導體產業", "IC 封測 (OSAT)")
                elif ticker_num in ["2408", "2344", "2337", "3006", "5351", "2451", "3260", "8271", "4973", "8277"] or has_kw(["dram", "flash memory", "sram", "eeprom", "nor flash"]):
                    main_cat, sub_cat = ("半導體產業", "記憶體 (DRAM/Flash)")
                elif ticker_num in ["3680", "3131", "3583", "6187", "3413", "6683", "6510", "6532", "5434", "3587", "1560", "6196", "3653", "3055", "3580", "8064", "7728", "6937", "8091"] or has_kw(["semiconductor equipment", "photolithography", "etching", "chemical mechanical", "probe card", "lead frame"]):
                    main_cat, sub_cat = ("半導體與 PCB 設備/材料", "半導體設備與材料")
                elif ticker_num in ["2481", "5425", "8261", "6435", "3317", "6525", "3675", "2302", "7712", "6720", "8255"] or has_kw(["mosfet", "diode", "igbt", "rectifier", "power semiconductor"]):
                    main_cat, sub_cat = ("半導體產業", "功率半導體 (MOSFET/二極體)")
                elif ticker_num in ["3105", "8086", "4971", "2455", "3707", "3016", "3221"] or has_kw(["gallium arsenide", "gaas", "rf ic", "radio frequency"]):
                    main_cat, sub_cat = ("半導體產業", "化合物半導體與射頻晶片")
                elif has_kw(["design", "fabless", "asic", "chipset"]) or has_kw(["設計", "DESIGN", "晶片", "矽", "IC"], in_summary=False):
                    if has_kw(["power management", "pmic", "analog ic"]):
                        main_cat, sub_cat = ("半導體產業", "IC 設計 - 類比與電源管理")
                    elif has_kw(["driver ic", "lcd driver"]):
                        main_cat, sub_cat = ("半導體產業", "IC 設計 - 顯示驅動 IC")
                    elif has_kw(["microcontroller", "mcu"]):
                        main_cat, sub_cat = ("半導體產業", "IC 設計 - MCU")
                    elif has_kw(["asic", "soc", "intellectual property", "ip core", "ip"]):
                        main_cat, sub_cat = ("半導體產業", "IC 設計 - ASIC/IP")
                    else:
                        main_cat, sub_cat = ("半導體產業", "IC 設計")
                else:
                    main_cat, sub_cat = ("半導體產業", "IC 設計")
            
            # 5. PCB & CCL
            elif has_kw(["printed circuit board", "pcb", "copper clad laminate", "ccl", "flexible printed", "fccl", "abf", "bt", "ic carrier", "carrier board"]) or has_kw(["電路板", "PCB", "載板", "CCL", "銅箔基板", "軟板", "硬板"], in_summary=False):
                if has_kw(["carrier board", "ic carrier", "ic substrate", "package substrate", "abf", "bt"]) or has_kw(["載板", "ABF", "BT"], in_summary=False):
                    main_cat, sub_cat = ("PCB 與銅箔基板", "IC 載板")
                elif has_kw(["ccl", "copper clad laminate"]) or has_kw(["銅箔", "CCL", "基板"], in_summary=False):
                    main_cat, sub_cat = ("PCB 與銅箔基板", "銅箔基板 (CCL)")
                elif has_kw(["flexible printed", "fpc", "fccl"]) or has_kw(["軟性", "FCCL", "軟板"], in_summary=False):
                    main_cat, sub_cat = ("PCB 與銅箔基板", "軟性銅箔基板 (FCCL)/軟板")
                else:
                    main_cat, sub_cat = ("PCB 與銅箔基板", "印刷電路板 (PCB)")
            
            # 6. Passive Components & Quartz
            elif has_kw(["resistor", "capacitor", "inductor", "choke", "varistor", "thermistor", "quartz", "oscillator", "crystal"]) or has_kw(["電阻", "電容", "電感", "被動", "陶瓷", "石英", "晶體", "振盪", "晶振"], in_summary=False):
                if has_kw(["quartz", "crystal", "oscillator", "resonator"]) or has_kw(["石英", "晶體", "振盪", "晶振"], in_summary=False):
                    main_cat, sub_cat = ("被動元件與石英元件", "石英元件")
                elif has_kw(["varistor", "thermistor", "fuse", "protection device"]) or has_kw(["保護元件", "防護", "保險絲"], in_summary=False):
                    main_cat, sub_cat = ("被動元件與石英元件", "被動元件 - 保護元件")
                elif has_kw(["electrolytic capacitor", "aluminum capacitor"]) or has_kw(["電解電容"], in_summary=False):
                    main_cat, sub_cat = ("被動元件與石英元件", "被動元件 - 電解電容")
                elif has_kw(["solid capacitor", "polymer capacitor"]) or has_kw(["固態電容"], in_summary=False):
                    main_cat, sub_cat = ("被動元件與石英元件", "被動元件 - 固態電容")
                elif has_kw(["inductor", "choke", "magnetic", "coil"]) or has_kw(["電感", "磁性"], in_summary=False):
                    main_cat, sub_cat = ("被動元件與石英元件", "被動元件 - 電感與磁性元件")
                else:
                    main_cat, sub_cat = ("被動元件與石英元件", "被動元件 (電阻/電容/電感)")
            
            # 3. LED & Optoelectronics
            elif has_kw(["led", "light-emitting diode", "optoelectronic", "display panel", "optical lens", "camera module"]):
                main_cat, sub_cat = ("光電與顯示面板", "光電與光學鏡頭")
            
            # 7. Cooling
            elif has_kw(["thermal module", "cooling fan", "heat sink", "vapor chamber", "heat pipe"]) or has_kw(["散熱", "風扇", "熱"], in_summary=False):
                main_cat, sub_cat = ("工業電腦與電腦週邊", "散熱模組與元件")
            
            # 8. Server Rails & Chassis
            elif (has_kw(["slide rail", "guide rail"]) and has_kw(["server"])) or has_kw(["server chassis"]) or has_kw(["導軌", "滑軌", "機殼"], in_summary=False):
                main_cat, sub_cat = ("伺服器與資訊週邊", "伺服器導軌與滑軌")
            
            # 9. Industrial PC (IPC)
            elif has_kw(["industrial computer", "industrial pc", "single board computer", "ipc"]) or has_kw(["工業電腦", "IPC"], in_summary=False):
                main_cat, sub_cat = ("工業電腦與電腦週邊", "工業電腦 (IPC)")
            
            # 10. Server / EMS / ODM
            elif has_kw(["notebook computer", "server manufacturing", "pcb assembly", "ems"]):
                main_cat, sub_cat = ("伺服器與資訊週邊", "伺服器與筆電代工")
            
            else:
                normalized_ind = raw_industry.replace("X", " - ").replace("\u2013", " - ").replace("\u2014", " - ").strip()
                if normalized_ind in DETAILED_SECTOR_MAP:
                    main_cat, sub_cat = DETAILED_SECTOR_MAP[normalized_ind]
                elif raw_industry in DETAILED_SECTOR_MAP:
                    main_cat, sub_cat = DETAILED_SECTOR_MAP[raw_industry]
                else:
                    main_cat = "其他未分類"
                    sub_cat = raw_industry
        
        if main_cat not in grouped:
            grouped[main_cat] = {}
        if sub_cat not in grouped[main_cat]:
            grouped[main_cat][sub_cat] = []
            
        grouped[main_cat][sub_cat].append((name, ticker))
        
    # 6. Generate Markdown Report
    print(f"Generating Master MD report: {REPORT_MD}")
    md_lines = [
        f"# 👑 全持股/觀測名單產業地圖大師版",
        f"此報告由 `classify_all_database.py` 自動生成。分析對象為 `{STOCK_LIST_FILE}` 中的所有個股，共計 **{len(details)}** 檔。\n",
        "## 📊 產業族群分類統計",
    ]
    
    cat_counts = {}
    for main_cat, sub_cats in grouped.items():
        cat_counts[main_cat] = sum(len(s_list) for s_list in sub_cats.values())
        
    for main_cat, count in sorted(cat_counts.items(), key=lambda x: -x[1]):
        md_lines.append(f"- **{main_cat}**：共 {count} 檔")
        
    md_lines.append("\n---\n")
    
    for main_cat in sorted(grouped.keys()):
        sub_cats = grouped[main_cat]
        total_in_cat = sum(len(s_list) for s_list in sub_cats.values())
        md_lines.append(f"## 📁 {main_cat}（共 {total_in_cat} 檔）\n")
        
        for sub_cat, stocks in sorted(sub_cats.items()):
            md_lines.append(f"### 🔍 {sub_cat}（{len(stocks)} 檔）")
            for name, ticker in sorted(stocks, key=lambda x: x[0]):
                md_lines.append(f"- **{name}** ({ticker})")
            md_lines.append("")
        md_lines.append("")
        
    with open(REPORT_MD, "w", encoding="utf-8") as f:
        f.write("\n".join(md_lines))
        
    # 7. Generate Master HTML Report
    print(f"Generating Master HTML report: {REPORT_HTML}")
    
    chart_bars = ""
    max_count = max(cat_counts.values()) if cat_counts else 1
    for main_cat, count in sorted(cat_counts.items(), key=lambda x: -x[1]):
        pct = (count / max_count) * 100
        chart_bars += f"""
        <div class="chart-row">
            <span class="chart-label">{main_cat}</span>
            <div class="chart-bar-container">
                <div class="chart-bar" style="width: {pct}%;"></div>
            </div>
            <span class="chart-value">{count} 檔</span>
        </div>
        """
        
    html_cards = ""
    for idx, main_cat in enumerate(sorted(grouped.keys())):
        sub_cats = grouped[main_cat]
        total_in_cat = sum(len(s_list) for s_list in sub_cats.values())
        
        sub_html = ""
        for sub_cat, stocks in sorted(sub_cats.items()):
            stock_li = "".join(f'<li><span class="stock-name">{name}</span> <span class="stock-ticker">{ticker}</span></li>' for name, ticker in sorted(stocks, key=lambda x: x[0]))
            sub_html += f"""
            <div class="sub-category">
                <div class="sub-header">🔍 {sub_cat} ({len(stocks)} 檔)</div>
                <ul class="stock-list">
                    {stock_li}
                </ul>
            </div>
            """
            
        html_cards += f"""
        <div class="card">
            <div class="card-header" onclick="toggleCard({idx})">
                <span class="card-title">📁 {main_cat}</span>
                <span class="badge">{total_in_cat} 檔</span>
            </div>
            <div class="card-body" id="card-body-{idx}">
                {sub_html}
            </div>
        </div>
        """

    html_template = f"""<!DOCTYPE html>
<html lang="zh-TW">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>👑 全持股/觀測名單全分類產業地圖</title>
    <style>
        :root {{
            --bg-color: #0b0f19;
            --card-bg: #151f32;
            --border-color: #233554;
            --text-color: #e2e8f0;
            --text-muted: #8892b0;
            --primary: #64ffda;
            --accent: #57cbff;
            --font: 'Outfit', 'Inter', -apple-system, sans-serif;
        }}
        
        body {{
            background-color: var(--bg-color);
            color: var(--text-color);
            font-family: var(--font);
            margin: 0;
            padding: 2rem;
            line-height: 1.6;
        }}
        
        .container {{
            max-width: 1400px;
            margin: 0 auto;
        }}
        
        header {{
            text-align: center;
            margin-bottom: 3rem;
        }}
        
        h1 {{
            font-size: 2.8rem;
            font-weight: 800;
            background: linear-gradient(135deg, var(--primary), var(--accent));
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
            margin-bottom: 0.5rem;
        }}
        
        .subtitle {{
            color: var(--text-muted);
            font-size: 1.1rem;
        }}
        
        .chart-section {{
            background-color: var(--card-bg);
            border: 1px solid var(--border-color);
            border-radius: 16px;
            padding: 2rem;
            margin-bottom: 3rem;
            box-shadow: 0 10px 30px -15px rgba(2, 12, 27, 0.7);
        }}
        
        .chart-title {{
            font-size: 1.4rem;
            font-weight: 700;
            margin-bottom: 1.5rem;
            color: var(--primary);
            border-left: 4px solid var(--primary);
            padding-left: 0.8rem;
        }}
        
        .chart-row {{
            display: flex;
            align-items: center;
            margin-bottom: 0.8rem;
        }}
        
        .chart-label {{
            width: 280px;
            font-weight: 600;
            font-size: 0.95rem;
        }}
        
        .chart-bar-container {{
            flex-grow: 1;
            background-color: #1d2d44;
            height: 16px;
            border-radius: 8px;
            overflow: hidden;
            margin: 0 1.5rem;
        }}
        
        .chart-bar {{
            background: linear-gradient(90deg, var(--primary), var(--accent));
            height: 100%;
            border-radius: 8px;
        }}
        
        .chart-value {{
            width: 80px;
            text-align: right;
            font-weight: 700;
            color: var(--primary);
        }}
        
        .grid {{
            display: grid;
            grid-template-columns: 1fr 1fr;
            gap: 1.5rem;
        }}
        
        @media (max-width: 1024px) {{
            .grid {{
                grid-template-columns: 1fr;
            }}
            .chart-row {{
                flex-direction: column;
                align-items: flex-start;
            }}
            .chart-label {{
                width: 100%;
                margin-bottom: 0.3rem;
            }}
            .chart-bar-container {{
                margin: 0.3rem 0;
                width: 100%;
            }}
        }}
        
        .card {{
            background-color: var(--card-bg);
            border: 1px solid var(--border-color);
            border-radius: 16px;
            overflow: hidden;
            box-shadow: 0 10px 30px -15px rgba(2, 12, 27, 0.5);
            transition: all 0.35s ease;
        }}
        
        .card:hover {{
            transform: translateY(-3px);
            border-color: var(--primary);
        }}
        
        .card-header {{
            background-color: rgba(2, 12, 27, 0.3);
            padding: 1.3rem 1.8rem;
            font-weight: 700;
            font-size: 1.25rem;
            display: flex;
            justify-content: space-between;
            align-items: center;
            cursor: pointer;
            user-select: none;
            border-bottom: 1px solid var(--border-color);
        }}
        
        .card-title {{
            display: flex;
            align-items: center;
            gap: 0.5rem;
        }}
        
        .badge {{
            background-color: rgba(100, 255, 218, 0.1);
            color: var(--primary);
            font-size: 0.85rem;
            padding: 4px 12px;
            border-radius: 20px;
            font-weight: 700;
            border: 1px solid rgba(100, 255, 218, 0.2);
        }}
        
        .card-body {{
            padding: 1.8rem;
            display: none; /* Default closed to keep it clean on start */
        }}
        
        .sub-category {{
            margin-bottom: 1.5rem;
            background-color: rgba(2, 12, 27, 0.2);
            padding: 1.2rem;
            border-radius: 8px;
            border-left: 3px solid var(--accent);
        }}
        
        .sub-category:last-child {{
            margin-bottom: 0;
        }}
        
        .sub-header {{
            font-weight: 700;
            font-size: 1.05rem;
            margin-bottom: 0.8rem;
            color: var(--accent);
        }}
        
        .stock-list {{
            list-style: none;
            padding: 0;
            margin: 0;
            display: grid;
            grid-template-columns: 1fr 1fr;
            gap: 0.5rem;
        }}
        
        @media (max-width: 600px) {{
            .stock-list {{
                grid-template-columns: 1fr;
            }}
        }}
        
        .stock-list li {{
            display: flex;
            justify-content: space-between;
            background-color: rgba(2, 12, 27, 0.1);
            padding: 6px 12px;
            border-radius: 6px;
            font-size: 0.9rem;
            border: 1px solid transparent;
            transition: all 0.2s;
        }}
        
        .stock-list li:hover {{
            background-color: rgba(2, 12, 27, 0.4);
            border-color: rgba(100, 255, 218, 0.3);
        }}
        
        .stock-name {{
            font-weight: 600;
        }}
        
        .stock-ticker {{
            color: var(--text-muted);
            font-family: monospace;
        }}
        
        .btn-toggle-all {{
            margin-bottom: 1.5rem;
            background: transparent;
            border: 1px solid var(--primary);
            color: var(--primary);
            padding: 8px 16px;
            border-radius: 6px;
            cursor: pointer;
            font-weight: 600;
            font-size: 0.9rem;
            transition: all 0.3s;
        }}
        
        .btn-toggle-all:hover {{
            background-color: rgba(100, 255, 218, 0.1);
        }}
    </style>
    <script>
        function toggleCard(idx) {{
            const body = document.getElementById('card-body-' + idx);
            if (body.style.display === 'block') {{
                body.style.display = 'none';
            }} else {{
                body.style.display = 'block';
            }}
        }}
        
        let allOpen = false;
        function toggleAll() {{
            allOpen = !allOpen;
            const bodies = document.querySelectorAll('.card-body');
            bodies.forEach(body => {{
                body.style.display = allOpen ? 'block' : 'none';
            }});
            document.getElementById('btn-toggle').innerText = allOpen ? '⚡ 全部摺疊 (Collapse All)' : '⚡ 全部展開 (Expand All)';
        }}
    </script>
</head>
<body>
    <div class="container">
        <header>
            <h1>👑 全持股/觀測名單全產業地圖大師版</h1>
            <div class="subtitle">分析時間: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")} | 共分類 {len(details)} 檔標的</div>
        </header>
        
        <!-- Summary Chart Section -->
        <section class="chart-section">
            <div class="chart-title">📊 全球/台股板塊分佈統計</div>
            {chart_bars}
        </section>
        
        <button id="btn-toggle" class="btn-toggle-all" onclick="toggleAll()">⚡ 全部展開 (Expand All)</button>
        
        <!-- Detailed Groups Grid -->
        <section class="grid">
            {html_cards}
        </section>
    </div>
</body>
</html>
"""
    with open(REPORT_HTML, "w", encoding="utf-8") as f:
        f.write(html_template)
        
    print("=" * 60)
    print("🎉 Pipeline Completed Successfully!")
    print(f"- Master Markdown Report: {REPORT_MD}")
    print(f"- Master HTML Report: {REPORT_HTML}")
    print("=" * 60)

if __name__ == "__main__":
    run_pipeline()
